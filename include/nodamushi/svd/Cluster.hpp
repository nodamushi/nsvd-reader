 ...
 */
#ifndef NODAMUSHI_SVD_CLUSTER_HPP
#define NODAMUSHI_SVD_CLUSTER_HPP
...
bool createRegisterOrCluster(SVD& c,Registers<STR>& d)
{
  ...
  else c.unknown_element(svd_element::Registers);
  return true;
}
}
...
bool create(SVD& svd,Cluster<STR>& d)
{
  while(svd)
  {
    ...
    else c.unknown_element(svd_element::Cluster);
    if(!t){
      c.illegal_value(svd_element::Cluster);
    }
  }
  return true;
}
...
#endif // NODAMUSHI_SVD_CLUSTER_HPP
